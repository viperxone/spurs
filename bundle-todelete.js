...
// Built: 2026-07-02T20:46:49.665Z
...
const SQUAD = [
  ...
  {name:"D. Spence",      pos:"RB",e:"🏴󠁧󠁢󠁥󠁮󠁧󠁿",apps:8, g:0,con:"2029",  wc:"ENG",  st:"SQUAD — #25 England",  sc:"green"},
  // MIDFIELDERS
  {name:"J. Palhinha",    pos:"DM",e:"🇵🇹",apps:30,g:3,con:"EXPIRED",wc:null,  st:"CONTRACT EXPIRED",     sc:"red"},
  {name:"A. Gray",        pos:"CM",e:"🏴󠁧󠁢󠁥󠁮󠁧󠁿",apps:29,g:2,con:"2030",  wc:null,   st:"CORE",                 sc:"green"},
  {name:"L. Bergvall",    pos:"CM",e:"🇸🇪",apps:26,g:3,con:"2031",  wc:null,   st:"TRANSFER REQUESTED",   sc:"red"},
  ...
  {name:"R. Bentancur",   pos:"CM",e:"🇺🇾",apps:14,g:1,con:"EXPIRED",wc:"URU", st:"CONTRACT EXPIRED",     sc:"red"},
  ...
  {name:"M. Kudus",       pos:"FW",e:"🇬🇭",apps:26,g:6,con:"2031",  wc:null,   st:"MISSED WC — INJURY",   sc:"red"},
...
const TRANSFER_BRIEFS = [
  ...
  {
    player:"Sandro Tonali", e:"🇮🇹", from:"Newcastle", fee:"100m+", like:98, st:"hot", date:"1 Jul",
    brief:"Personal terms fully AGREED — 6-year contract worth £275k/week. Spurs have said yes to Newcastle's 100m+ valuation, which would obliterate the club transfer record set by Fernandes days earlier. TEAMtalk and Sky Sports both confirm terms are settled; only the club-to-club fee needs finalising. Man City reportedly trying to hijack but Spurs seen as favourites given the De Zerbi connection. Medical expected once clubs agree."
  },
  {
    player:"Savinho", e:"🇧🇷", from:"Man City", fee:"60m", like:50, st:"warm", date:"24 Jun",
    brief:"Talks with City active. Manager has given explicit approval. All attacking recruitment focus on the Brazilian. City willing to sell. May be delayed until Fernandes and Tonali are fully concluded."
  },
  {
    player:"Carlos Baleba", e:"🇨🇲", from:"Brighton", fee:"45m", like:45, st:"warm", date:"20 Jun",
    brief:"Manager worked with Baleba at Brighton. Wants reunion at N17. Fee could be negotiated down given relationship. Third midfield target behind Fernandes and Tonali."
  },
  {
    player:"Bart Verbruggen", e:"🇳🇱", from:"Brighton", fee:"30m", like:45, st:"warm", date:"20 Jun",
    brief:"Dubravka signing seen as short-term only. Still want a top-level GK. Manager's previous No.1 at Brighton. Vicario sale expected to fund this move."
  },
  {
    player:"Luka Vuskovic", e:"🇭🇷", from:"Spurs (academy)", fee:"45m bid from Brighton", like:80, st:"hot", date:"1 Jul",
    brief:"Brighton have submitted a third official bid worth £45m including add-ons per Romano. Spurs twice rejected previous bids. Club now considering accepting per reports — decision imminent. 19-year-old Croatian CB who has never played a senior Spurs minute."
  },
  {
    player:"Cody Gakpo", e:"🇳🇱", from:"Liverpool", fee:"60m+", like:35, st:"cold", date:"25 Jun",
    brief:"Identified as attacking option internally. Liverpool open to selling but have raised valuation to 60m+. Any deal likely delayed until World Cup ends. Netherlands in R32 vs Morocco."
  },
];

const CONFIRMED = [
  {
    player:"Sandro Tonali", e:"🏳️", from:"Newcastle", fee:"100m", role:"TBC", date:"2026-07-02",
    note:"Confirmed by BBC, Sky Sports, football.london and Romano. Tonali photographed boarding flight to complete transfer. 6-year deal reported. De Zerbi connection central. Previous TRANSFER_BRIEFS entry at 98% likelihood should now be removed and this entry replaces the provisional CONFIRMED entry — verify role/flag manually. — auto-added, verify flag/role manually"
  },
  {
    player:"Sandro Tonali", e:"🏳️", from:"Newcastle", fee:"100m", role:"TBC", date:"2026-07-02",
    note:"Club record (now surpassed by this deal post-Fernandes). BBC, Sky Sports, football.london and Romano all confirm agreement. Tonali photographed boarding flight to complete transfer. 6-year deal reported. De Zerbi connection central to move. — auto-added, verify flag/role manually"
  },
  // Newest first
  {
  ...
  },
  {
    ...
    note:"Players Player of Year at Boro. 4-yr deal."
  },
  {
...
const DEPARTURES = [
  {player:"Luka Vuskovic", note:"BBC headline confirms Brighton agree club record £46m Vuskovic deal. Already listed in DEPARTURES but fee now confirmed at £46m via BBC — update fee figure in DEPARTURES note if not already done."},
  {player:"Luka Vuskovic", note:"Sale to Brighton agreed — fee reported at £46m (BBC) to £50m (football.london) with a significant sell-on clause. 19-year-old Croatian CB had never made a senior Spurs appearance."},
  ...
  {player:"G. Vicario",      note:"Sale expected — Juve & Napoli interested (Romano)"},
  {player:"J. Palhinha",     note:"Contract expired Jun 30 — future uncertain"},
...
const ANON_BRIEFS = [
  ...
  {
    date:"28 Jun",
    text:"Sources suggest Bergvall's exit is being treated internally as a blessing — De Zerbi was always planning to reshape the midfield and the transfer fee received will help fund the Tonali pursuit."
  },
  {
    date:"25 Jun",
...
const NEWS = [
  {title:"DONE DEAL: Mateus Fernandes signs for Tottenham — club record £85m from West Ham. Romano + Ornstein confirmed. Medical complete.", source:"Romano / Ornstein", date:"1 Jul 2026", tag:"Official", url:"https://x.com/FabrizioRomano"},
  {title:"Luka Vuskovic: Brighton submit third bid of £45m for Spurs academy CB — club considering accepting (Romano)", source:"Fabrizio Romano", date:"1 Jul 2026", tag:"Transfer", url:"https://x.com/FabrizioRomano"},
  {title:"Tonali personal terms AGREED with Spurs — Newcastle now deciding whether to accept bid (Romano)", source:"Fabrizio Romano", date:"1 Jul 2026", tag:"Transfer", url:"https://x.com/FabrizioRomano"},
  ...
  {title:"Bergvall submits transfer request — wants out after just 112 minutes under De Zerbi", source:"Sky Sports", date:"27 Jun 2026", tag:"Transfer", url:"https://www.skysports.com/football/news"},
  {title:"Martin Dubravka confirmed — fourth signing joins July 1 on free from Burnley", source:"THFC Official", date:"24 Jun 2026", tag:"Official", url:"https://www.tottenhamhotspur.com/news"},
  {title:"Vicario set to leave — Juventus, Napoli and Aston Villa all monitoring situation", source:"Fabrizio Romano", date:"24 Jun 2026", tag:"Transfer", url:"https://x.com/FabrizioRomano"},
  {title:"2026/27 EPL fixtures: Spurs open at Brentford 22 Aug — NLD home Dec 5 vs Arsenal", source:"THFC Official", date:"19 Jun 2026", tag:"Fixtures", url:"https://www.tottenhamhotspur.com/fixtures"},
  {title:"Ben Davies 1-year extension confirmed — enters his 13th season as a Spur", source:"THFC Official", date:"10 Jun 2026", tag:"Official", url:"https://www.tottenhamhotspur.com/news"},
  {title:"Bissouma officially released — squad overhaul begins as De Zerbi reshapes the group", source:"THFC Official", date:"10 Jun 2026", tag:"Official", url:"https://www.tottenhamhotspur.com/news"},
  ...
  {title:"Robertson and Senesi complete medicals — Spurs summer rebuild gathering real pace", source:"THFC Official", date:"3 Jun 2026", tag:"Official", url:"https://www.tottenhamhotspur.com/news"},
];

// RSS sources — ALL fetched simultaneously on every page load.
// These sites carry breaking news from Romano, Ornstein, O'Keefe, Jacobs, Moretto, Szy
...
// CORS proxy for browser-based RSS fetching. Free, no API key needed.
...
// VERIFIED SQUADS — Official FIFA/FA/CBF sources, confirmed June 2026.
// CONFIRMED SPURS PLAYERS AT WC: Romero (ARG), Van de Ven (NED), Van Hecke (NED),
//   Danso (AUT), Porro (ESP), Spence (ENG), Kudus (GHA), Bentancur (URU), Sarr (SEN)
// NOT IN WC SQUADS: Maddison (not selected by Tuchel), Solanke (not selected),
//   Gallagher (not selected), Richarlison (not selected by Ancelotti — opted for youth)

const WC = [
  {
    name:"C. Romero", e:"🇦🇷", pos:"CB", country:"Argentina", group:"J",
    results:["W 3-0 Algeria","W 2-0 Austria","W 3-1 Jordan"],
    gp:3, xi:2, sub:0, bench:1, goals:0, ast:0, rtg:"6.9",
    progress:"R32 — Jul 3 vs Cape Verde", out:false,
    note:"Started vs Algeria (clean sheet). Forced off 57min vs Austria (knee knock). Benched vs Jordan as precaution. Knee tests negative — expected to start R32."
  },
  {
    name:"M. van de Ven", e:"🇳🇱", pos:"CB", country:"Netherlands", group:"F",
    results:["D 2-2 Japan","W 5-1 Sweden","W 3-1 Tunisia"],
    gp:3, xi:3, sub:0, bench:0, goals:0, ast:1, rtg:"7.4",
    progress:"R32 — Jun 29 vs Morocco", out:false,
    note:"Nailed-on starter for Netherlands across all 3 games. Assist vs Sweden (cut-back for Gakpo). Outstanding WC form — best Spurs performer at the tournament so far."
  },
  {
    name:"JP van Hecke", e:"🇳🇱", pos:"CB", country:"Netherlands", group:"F",
    results:["D 2-2 Japan","W 5-1 Sweden","W 3-1 Tunisia"],
    gp:2, xi:1, sub:0, bench:1, goals:0, ast:0, rtg:"7.1",
    progress:"R32 — Jun 29 vs Morocco", out:false,
    note:"Started vs Sweden alongside Van de Ven. Benched vs Japan and Tunisia. Solid partnership with Van de Ven when paired. Already a Spurs player — joins Jul 1."
  },
  {
    name:"P. Porro", e:"🇪🇸", pos:"RB", country:"Spain", group:"H",
    results:["D 0-0 Cape Verde","W 4-0 Saudi Arabia","W 1-0 Uruguay"],
    gp:3, xi:2, sub:1, bench:0, goals:0, ast:1, rtg:"7.1",
    progress:"R32 — Jul 2 vs Austria", out:false,
    note:"Started vs Cape Verde and Saudi Arabia. Came off bench in final group game (load management). Assist vs Saudi Arabia. Spain looking strong."
  },
  {
    name:"K. Danso", e:"🇦🇹", pos:"CB", country:"Austria", group:"J",
    results:["W 3-1 Jordan","L 0-2 Argentina","D 3-3 Algeria"],
    gp:3, xi:3, sub:0, bench:0, goals:0, ast:0, rtg:"6.5",
    progress:"R32 — Jul 2 vs Spain", out:false,
    note:"Ever-present starter for Austria. Tough night vs Argentina. Dramatic 3-3 vs Algeria saw Austria through as 3rd-place. Faces Spain in a brutal R32 draw."
  },
  {
    name:"D. Spence", e:"🏴󠁧󠁢󠁥󠁮󠁧󠁿", pos:"RB", country:"England", group:"L",
    results:["W 4-2 Croatia","D 0-0 Ghana","W 2-0 Panama"],
    gp:2, xi:1, sub:1, bench:1, goals:0, ast:0, rtg:"6.8",
    progress:"R32 — Jul 1 vs DR Congo", out:false,
    note:"ONLY Spurs player in England's 26-man squad (shirt #25). Off bench vs Croatia. Started vs Panama with Reece James rested. Benched vs Ghana."
  },
  {
    name:"R. Bentancur", e:"🇺🇾", pos:"CM", country:"Uruguay", group:"H",
    results:["D 1-1 S.Arabia","D 2-2 Cape Verde","L 0-1 Spain"],
    gp:3, xi:3, sub:0, bench:0, goals:0, ast:1, rtg:"6.7",
    progress:"ELIMINATED — Group H (3rd)", out:true,
    note:"Uruguay's starting CM all 3 games. Assist vs Cape Verde. Solid individually but Uruguay failed to progress. Out on goal difference. Contract at Spurs expires Jun 30."
  },
  {
    name:"PM. Sarr", e:"🇸🇳", pos:"CM", country:"Senegal", group:"I",
    results:["L 1-3 France","L 2-3 Norway","W 5-0 Iraq"],
    gp:3, xi:3, sub:0, bench:0, goals:1, ast:1, rtg:"7.2",
    progress:"ELIMINATED — Group I (3rd)", out:true,
    note:"Senegal's best player despite early exit. Scored vs Iraq. Assist vs Norway. Battled hard but France and Norway too strong. Hugely impressive individually."
  },
];
...
function NewsPanel(){
  ...
  useEffect(()=>{
    ...
    const parseFeed=(txt,sourceName)=>{
      ...
      return entries.map(e=>{
        ...
        return {title,source:sourceName,date:dateStr,tag:parseTag(title),url:link,ts:d.getTime()};
      }).filter(i=>i.title.length>10);
    };

    // Fire all feeds simultaneously
    const fetches=RSS_SOURCES.map(src=>
      fetch(CORS_PROXY+encodeURIComponent(src.url),{signal:AbortSignal.timeout(8000)})
        .then(r=>r.text())
        .then(txt=>({ok:true,items:parseFeed(txt,src.name),name:src.name}))
        .catch(()=>({ok:false,items:[],name:src.name}))
    );
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
          ...
          {!loading&&!live&&(
            <>
              ...
              <span style={{fontSize:11,color:P.amber,fontWeight:700}}>OFFLINE — showing cached headlines</span>
            </>
          )}
        </div>
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      <div style={{background:P.transferBg,border:`1px solid ${P.transferBorder}44`,
        ...
        <div style={{display:'flex',flexDirection:'column',gap:6}}>
          {/* Show confirmed first as a special item */}
          <div style={{padding:'9px 10px',background:P.bgCard,borderRadius:4,
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:3}}>
              <span style={{fontSize:13,fontWeight:800,color:P.white}}>🇵🇹 M. Fernandes</span>
              <span style={{fontSize:10,background:P.green,color:P.bg,padding:'2px 6px',
                borderRadius:3,fontWeight:900}}>DONE ✓</span>
            </div>
            <div style={{fontSize:11,color:P.green,fontWeight:700}}>£85m club record — West Ham</div>
            <div style={{fontSize:10,color:P.muted,marginTop:2}}>Romano + Ornstein confirmed · 1 Jul</div>
          </div>
      ...
      <button className="news-flash" onClick={onNews} style={{padding:'12px 14px',borderRadius:6,
        ...
        <div style={{fontSize:11,color:P.muted,marginTop:4}}>
          {NEWS.length}+ articles · Fetched fresh every load · Last: {now}
        </div>
      </button>
